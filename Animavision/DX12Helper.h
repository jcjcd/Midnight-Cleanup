...
inline void ThrowIfFailed(HRESULT hr)
{
	if (FAILED(hr))
	{
		OutputDebugStringA((HrToString(hr) + "\n").c_str());
...
inline void GetSoftwareAdapter(IDXGIFactory2* pFactory, IDXGIAdapter1** ppAdapter)
{
	IDXGIAdapter1* adapter = nullptr;
	*ppAdapter = nullptr;
...
inline void SetDebugLayerInfo(ID3D12Device* pD3DDevice)
{
...
inline std::wstring GetLatestWinPixGpuCapturerPath_Cpp17()
{
	...
	if (!std::filesystem::exists(pixInstallationPath) || !std::filesystem::is_directory(pixInstallationPath)) 
	{
		return L"PIX IS NOT INSTALLED";
	}
