...
struct VideoTexture::InternalData {
    ...
    std::shared_ptr<DX11Texture> target_texture = nullptr;
    uint32_t  width = 0;
    uint32_t  height = 0;
    float     fps = 0.0f;
...
public:
    ...
    bool open(const char* filename, Renderer* renderer) {
        finished = false;
       ...
       /* CHECK_HR(MFTRegisterLocalByCLSID(
        ...
        update(renderer, 0.0f);

        return true;
    }

    void update(Renderer* renderer, float elapsed) {
        ...
        if (pVideoSample) {
            ...
            if (!target_texture) {
                ...
                target_texture = std::make_shared<DX11Texture>(renderer->GetContext(), "VideoTexture", width, texture_height, Texture::Format::R8_UNORM, true);
            }

            Clock clock;
            target_texture->UpdateFromIYUV(renderer->GetContext(), byteBuffer, buffCurrLen);
...
bool VideoTexture::create(const char* filename, Renderer* _renderer)
{
    assert(!internal_data);
    internal_data = new InternalData();
	renderer = _renderer;
	return internal_data->open(filename, renderer);
}
...
{
    internal_data->update(renderer, dt);
