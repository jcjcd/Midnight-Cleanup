...
namespace GpuKernels
{
		...
		UINT numInvocationsPerFrame)
	{
		// Create shader resources
		{
			...
			for (UINT i = 0; i < m_readbackResources.size(); i++)
				...
				{
					std::wstringstream wResourceName;
		...
		UINT invocationIndex)   // per frame invocation index)
	{
		...
		SIZE_T readBackBaseOffset = frameIndex * m_resultSize;
		{
			...
			commandList->SetComputeRootDescriptorTable(Slot::Output, m_csReduceSumOutputs[0]->GetGpuDescriptorHandleUAV());

			for (UINT i = 0; i < m_csReduceSumOutputs.size(); i++)
			{
				auto outputResourceDesc = m_csReduceSumOutputs[i]->GetTexture()->GetDesc();
				...
				{
					m_csReduceSumOutputs[i]->ResourceBarrier(commandList, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
					commandList->SetComputeRootDescriptorTable(Slot::Input, m_csReduceSumOutputs[i]->GetGpuDescriptorHandle());
					commandList->SetComputeRootDescriptorTable(Slot::Output, m_csReduceSumOutputs[i + 1]->GetGpuDescriptorHandleUAV());
				}
				else  // We're done, prepare the last output for copy to readback.
				{
					commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_csReduceSumOutputs.back()->GetTexture(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE));
				}
			}

			// Copy the sum result to the readback buffer.
			auto destDesc = m_readbackResources[invocationIndex]->GetDesc();
			auto srcDesc = m_csReduceSumOutputs.back()->GetTexture()->GetDesc();
			...
			CD3DX12_TEXTURE_COPY_LOCATION copySrc(m_csReduceSumOutputs.back()->GetTexture(), 0);
			...
			// Transition the intermediate output resources back.
			{
				...
				{
					barriers[i] = CD3DX12_RESOURCE_BARRIER::Transition(m_csReduceSumOutputs[i]->GetTexture(), D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
				}
				barriers[m_csReduceSumOutputs.size() - 1] = CD3DX12_RESOURCE_BARRIER::Transition(m_csReduceSumOutputs.back()->GetTexture(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
