...
ConstantBuffer<PrimitiveMaterialBuffer> l_material : register(b0, space1);
StructuredBuffer<Index> l_indices : register(t0, space1);
StructuredBuffer<BasicVertex> l_vertices : register(t1, space1);             // Current frame vertex buffer.
StructuredBuffer<BasicVertex> l_verticesPrevFrame : register(t2, space1); 

Texture2D<float3> l_texDiffuse : register(t3, space1);
Texture2D<float3> l_texNormalMap : register(t4, space1);
Texture2D<float3> l_MetallicMap : register(t5, space1);
Texture2D<float3> l_RoughnessMap : register(t6, space1);
Texture2D<float3> l_OpacityMap : register(t7, space1);
ConstantBuffer<Offsets> l_offsets : register(b1, space1);
    ...
    in PrimitiveMaterialBuffer material)
{
    ...
    float3 albedo = l_texDiffuse.SampleLevel(LinearWrapSampler, uv, 0).xyz;
    float metallic = l_MetallicMap.SampleLevel(LinearWrapSampler, uv, 0).x;
    float roughness = l_RoughnessMap.SampleLevel(LinearWrapSampler, uv, 0).x;
    float opacity = l_OpacityMap.SampleLevel(LinearWrapSampler, uv, 0).x;
                                                
                                     
    float3 Kd = albedo;
    float3 H = normalize(V + WorldRayDirection());      
    float3 Ks = 0.04;
    Ks = fresnelSchlickRoughness(dot(H, V), Ks, roughness);
    float3 Kr = Ks;
    float3 Kt = tofloat3(1.0f - opacity);
...
void MyRayGenShader_RadianceRay()
{
    ...
    PathtracerRayPayload rayPayload = TraceRadianceRay(ray, currentRayRecursionDepth, g_cb.Znear, g_cb.Zfar);
    ...
    in BuiltInTriangleIntersectionAttributes attr)
{
    ...
    else 
    {
        ...
        float2 uv0 = vertices[0].uv;
        float2 uv1 = vertices[1].uv;
        float2 uv2 = vertices[2].uv;
...
void MyClosestHitShader_RadianceRay(inout PathtracerRayPayload rayPayload, in BuiltInTriangleIntersectionAttributes attr)
{
    ...
    const uint offset = l_offsets.vertexOffset;
    const uint3 offsetedIndices = indices + offset;
                                                
    BasicVertex vertices[3] = {
        l_vertices[offsetedIndices[0]],
        l_vertices[offsetedIndices[1]],
        l_vertices[offsetedIndices[2]] };

    float2 vertexTexCoords[3] = { vertices[0].uv, vertices[1].uv, vertices[2].uv };
    float2 texCoord = HitAttribute(vertexTexCoords, attr);

    //UINT materialID = l_materialCB.materialID;
    PrimitiveMaterialBuffer material = l_material;
    ...
    rayPayload.radiance = ShadeFromTexture(rayPayload, normal, objectNormal, hitPosition, texCoord, material);
}
