...
PixelOut PSMain(VertexOut pin, bool isFrontFace : SV_IsFrontFace)  // SV_IsFrontFace를 픽셀 셰이더의 입력으로 받음
{
    ...
    matrix worldinv = transpose(gWorldInvTranspose);
    float3 modelPos = mul(float4(pin.PosW, 1.0f), worldinv).xyz;
    float3 degree45normal = normalize(float3(1.0f, 1.0f, 1.0f));
    

    clip(dot(modelPos, degree45normal) - 0.5f);  // 45도 이상의 각도로 기울어진 폴리곤을 제거)
    return pout;
}