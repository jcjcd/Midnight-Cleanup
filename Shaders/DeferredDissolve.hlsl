...
cbuffer cbDissolveFactor
{
    float3 gDissolveColor;
...
PixelOut PSMain(VertexOut pin, bool isFrontFace : SV_IsFrontFace)  // SV_IsFrontFace를 픽셀 셰이더의 입력으로 받음
{
    ...
    // 디졸브 팩터를 계산한다.
    float noise = gDissolveNoise.Sample(gsamAnisotropicWrap, pin.TexC).r;
    
    clip(noise - gDissolveFactor);  // 디졸브 팩터를 사용하여 클리핑한다.
    ...
    //float3 dissolveColor = gDissolveColor * step(noise - gDissolveFactor, gEdgeWidth);
    float3 dissolveColor = gDissolveColor * smoothstep(gDissolveFactor - gEdgeWidth, gDissolveFactor, noise);

    pout.Emissive = float4(dissolveColor, 1.0f);
